...
*/
  user_settings = await get_UserSettings()
  //aianswer = await get_aiAnswer(emotion, textInput, user_settings.a_attitude, user_settings.composition, user_settings.intensity, user_settings.length, userData.name, userData.introduce)
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log("Introduce:", userData.introduce);
      ...
      console.log("답변 태도: ", user_settings.a_attitude);
      console.log("문장 구성: ", user_settings.composition);
      console.log("답변 강도: ", user_settings.intensity);
      console.log("답변 길이: ", user_settings.length);
      console.log("AI answer: ", aianswer);

      console.log("user 디버깅 name:", user.name);
      console.log("user 디버깅 introduce: ", user.introduce)
      console.log("user 디버깅 emotion: ", user.emotions)
...
async function get_aiAnswer(user_emotion, user_text, us_attitude, us_composition, us_intensity, us_length, us_name, us_introduce) {
  const requestData = {
    text: String(user_text),
    emotion: String(user_emotion),
    a_attitude: String(us_attitude),
    composition: Array.isArray(us_composition) ? us_composition : [us_composition],
    intensity: parseFloat(us_intensity),
    length: parseInt(us_length, 10),
    name: String(us_name),
    introduce: String(us_introduce)
  };
...
async function get_UserSettings() {
  try {
    const response = await fetch('/user_settings');  // GET 요청
